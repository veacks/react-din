...
import { useAudioGraphStore, type TremoloNodeData } from '../store';
import { audioEngine } from '../AudioEngine';
import { formatConnectedValue, useTargetHandleConnection } from '../paramConnections';

const TremoloNode = memo(({ id, data, selected }: NodeProps) => {
    ...
    const handleChange = (key: keyof TremoloNodeData, value: number | string | boolean) => {
        updateNodeData(id, { [key]: value });
        audioEngine.updateNode(id, { [key]: value });
    };

    return (
        <div className={`audio-node tremolo-node ${selected ? 'selected' : ''}`}>
            <div className="node-header" style={{ justifyContent: 'space-between', position: 'relative', background: '#7a9cff' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span className="node-title">Tremolo</span>
                </div>
            ...
            <div className="node-content">
                <div className="node-control">
                    ...
                    {rateConnection.connected ? (
                        ...
                        <input type="range" min="0.1" max="20" step="0.1" value={tremoloData.rate} onChange={(e) => handleChange('rate', Number(e.target.value))} />
                    )}
                ...
                <div className="node-control">
                    ...
                    {depthConnection.connected ? (
                        ...
                        <input type="range" min="0" max="1" step="0.01" value={tremoloData.depth} onChange={(e) => handleChange('depth', Number(e.target.value))} />
                    )}
                    <Handle type="target" position={Position.Left} id="depth" className="handle handle-in handle-param" />
                </div>
                <div className="node-control">
                    <label>Waveform</label>
                    <select value={tremoloData.waveform} onChange={(e) => handleChange('waveform', e.target.value)}>
                ...
                <div className="node-control">
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <input type="checkbox" checked={tremoloData.stereo} onChange={(e) => handleChange('stereo', e.target.checked)} />
                        Stereo
                    </label>
                </div>
                <div className="node-control">
                    ...
                    {mixConnection.connected ? (
                        ...
                        <input type="range" min="0" max="1" step="0.01" value={tremoloData.mix} onChange={(e) => handleChange('mix', Number(e.target.value))} />
                    )}
...
TremoloNode.displayName = 'TremoloNode';
