...
## Purpose
Load and trigger audio files directly from the editor.

## Props / Handles
- Data fields: `src`, `assetPath`, `loop`, `playbackRate`, `detune`, `loaded`, `sampleId`, `fileName`, `label`.
...
- Imported patch assets stay intentionally unresolved in the editor and remain silent until they are relinked to a local library entry.
